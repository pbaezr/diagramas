...
Es = zeros(1,m); % rigidez tangente o pendiente en la curva tensión-deformación del acero

% calcular para cada capa de refuerzo (generalmente se simplifica considerando una capa inferior y otra superior)
if any(opcionAcero{1} == [1 3]) % modelo elastoplástico o de Menegotto y Pinto    
    ef = parametros.ef; % máxima deformación unitaria permitida
    Es2 = 0; % módulo de elasticidad después tramo lineal-elástico del acero (en MPa)
    if opcionAcero{1} == 3, Es2 = Es1*parametros.E1; end % si el modelo no es elasto-plástico perfecto, Es2>0
...
Ec = zeros(1,n); % rigidez tangente o pendiente en la curva esfuerzo-deformación del hormigón
